...
def seed_data():
    with app.app_context():
        ...
        if not admin_password or not user_password or not manager_password:
            raise EnvironmentError(
                "Missing required environment variables: SEED_ADMIN_PASSWORD, SEED_USER_PASSWORD, SEED_MANAGER_PASSWORD"
            )
        ...
        u1.set_password(admin_password, app.config["BCRYPT_ROUNDS"])
        u1.role = "admin"
        db.session.add(u1)
        ...
        u2.set_password(user_password, app.config["BCRYPT_ROUNDS"])
        u2.role = "user"
        db.session.add(u2)
        ...
        u3.set_password(manager_password, app.config["BCRYPT_ROUNDS"])
        u3.role = "manager"
        db.session.add(u3)
